...
public class PayPsdInputView extends EditText {
    ...
    private void getAtt(AttributeSet attrs) {
        ...
        mInputCount = typedArray.getInt(R.styleable.PayPsdInputView_input_count, 2);
...
//       super.onDraw(canvas);
        ...
        if (mInputCount == 2) {
            if (getText().length() == maxCount) {
                ++count;
                if (count == 1) {
                    //记录密码
                    mComparePassword = getText().toString();
                    if (mOnSettingListener != null) {
                        mOnSettingListener.onOneInputComplet(mComparePassword);
                    }
                }
                if (count >= 2) {
                    //回调出去
                    String newPwd = getText().toString();
                    if (mOnSettingListener != null) {
                        if (mComparePassword.equals(newPwd)) {
                            mOnSettingListener.onEqual(newPwd);
                        } else {
                            mOnSettingListener.onError(count - 1);
                        }
    ...
    public interface OnSettingListener {
        ...
        void onOneInputComplet(String psd);
